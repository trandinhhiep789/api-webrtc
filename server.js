...
peersOnlineUser.on('connection', socket => {
  ...
  socket.on("sendLinkToCall", ({ senderId, senderUsername, receiverId, link }) => {
    console.log(senderId, senderUsername, receiverId, link)
    // const user = getUserById(receiverId)
    peersOnlineUser.to(receiverId).emit("getLinkToCall", { senderId, senderUsername, link })
  })

  socket.on("busyLinkToCall", ({ receiverId, isBusy }) => {
    console.log("busyLinkToCallbusyLinkToCallbusyLinkToCallbusryLinkToCallbusyLinkToCallbusyLinkToCallbusyLinkToCallbusyLinkToCallbusyLinkToCall")
    // const user = getUserById(receiverId)
    peersOnlineUser.to(receiverId).emit("getBusyLinkToCall", { isBusy })
  })

  socket.on("userReqEndTheCall", ({ receiverId, isEnd }) => {
    console.log(receiverId, isEnd)
    // const user = getUserById(receiverId)
    peersOnlineUser.to(receiverId).emit("userResEndTheCall", { isEnd })
  })
